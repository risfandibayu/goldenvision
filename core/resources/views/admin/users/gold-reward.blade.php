                            ...
                            <tbody>
                                @forelse($users as $user)
                                    <tr>
                                        ...
                                        <td data-label="@lang('Daily Gold')">{{ userGoldID($user->id)['daily'] }} gr</td>
                                        <td data-label="@lang('Weekly Gold')">{{ userGoldID($user->id)['weekly'] }} gr</td>
                                        <td>
                                            {!! $user->wd_gold
                                        ...
                                        {{-- <td data-label="@lang('Action')">
                                            @if ($user->total_weekly_golds < 0.5 && $user->daily_golds_count == 100)
                                                <a href="{{ route('admin.users.reward.add-gold', $user->id) }}"
                                                    class="icon-btn" data-toggle="tooltip"
                                                    data-original-title="@lang('Give Gold Reward')">
...
@push('breadcrumb-plugins')
    <div class="row">
        ...
        <div class="col-md-10 col-9">
            <form action="{{ route('admin.users.reward.gold') }}" method="GET"
                class="form-inline float-sm-right bg--white mr-2">
